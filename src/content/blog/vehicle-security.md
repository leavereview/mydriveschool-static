...
howTo:
  name: "How to secure your vehicle when parking or leaving it"
  description: "Step-by-step guidance for safely stopping, securing, and leaving a vehicle — for UK learner drivers and the driving test"
  steps:
    - name: "Apply the handbrake whenever you stop"
      text: "Every time the car comes to a complete stop — at traffic lights, when waiting, when parking — apply the handbrake. In an automatic, select Park (P) instead of or in addition to the handbrake. This prevents the vehicle rolling if your foot slips or you are struck from behind."
    - name: "Select neutral (or Park) before switching off"
      text: "In a manual car, select neutral before switching off the engine. In an automatic, select Park (P). Leaving the car in gear without the engine running is not a safety substitute for the handbrake, though on steep hills you may want both as a precaution."
    - name: "Switch off the engine and remove the key"
      text: "Turn the ignition off and remove the key. In a push-button start vehicle, ensure the engine is off before opening the door. In the UK, leaving your engine running unattended on a public road is a fixed penalty offence under the Road Traffic Act."
    - name: "Check mirrors before opening the door"
